...
class Robot():
	def __init__(self, game_display, map, position_initiale, name="Player 1"):
		...
		self.vitesse_max = 25 # pixel/image -> depend des fps donc des performances des pc qui sont inégales !
	...
	def move(self):
		...
		if self.alive == True:
			...
			message = "vitesse : ({:7.5n}, {:7.5}".format(float(self.vitesse[0]), float(self.vitesse[1]))
